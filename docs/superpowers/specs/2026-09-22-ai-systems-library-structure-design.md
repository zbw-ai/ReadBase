...
> 日期：2026-09-22。
> 状态：用户已确认总体结构及五项补充；本文件将其整理为待审阅的落地规格，尚未重组正式入口。
...
六个 Part 是学习视图，不是互斥的学科分类：具身是综合应用路径，复用训练、推理、RL 及公共基础；实验为所有方向服务。
...
拟新增的轻量 Part 导航放在现有 `roadmaps/` 中：

```text
ReadBase/
  README.md                                  # 首页：当前重点＋六个 Part＋快捷入口
  ...
  private_resume/                             # 保留面试主文档和 Coding 快捷入口
```

新增的五份 README 都是薄导航，不复制 `topics/`、不拥有独立扫描或队列；没有匹配内容的项写明“待建设”，不创建空目录或死链接。
...
3. 六个 Part 的简短表格：解决什么问题、从哪里开始。
4. 常用入口：知识关系、资料索引、最新追踪、实验、排障、面试速查。
5. 简短维护原则；详细目录职责移至手册入口。
...
| 文件或区域 | 改动 |
|---|---|
| 根 README、手册 README、philosophy | 更新定位、当前重点、六个 Part；去掉互相等待的 Phase 描述 |
...
| AGENTS、CLAUDE | 同步定位、目录职责和未来扫描范围，保留既有发布/来源核验约束 |
...
- 首页可直接进入六个 Part；由 Part 到主正文通常再点击一次。
...
本轮只落地这份结构与课程规格。官方来源已做针对性核对，但没有执行全量 frontier scan，没有改动主 README、Part、队列、追踪规则或实验入口，也没有运行任何 GPU 实验。
