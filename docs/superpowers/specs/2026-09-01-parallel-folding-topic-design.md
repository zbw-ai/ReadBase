...
| PP/VPP | `MEGATRON-01/07` | 对均衡、忽略通信的 non-interleaved 1F1B 基线，`bubble/useful=(p-1)/m`，总时间占比为 `(p-1)/(m+p-1)`；interleaved 1F1B/VPP 在 forward/backward 与 `v` 个 model chunks 近似均衡等额外假设下，理想 bubble 约缩小 `v` 倍；同时保留 microbatch/layer divisibility 或 custom pipeline layout 条件，以及增加 P2P/调度/负载不均的代价 | Megatron 论文/用户要求 | 把理想公式用于不均衡 stage；microbatch 越多永远越好；忽略 layout 整除约束 | 已有，补条件 |
