...
> - 修订日期：2026-09-18；官方资料的核验日期与项目版本见文末
...
- **P1**：[RESUME-11 第二个 Ownership 案例](#resume-11) · [RESUME-16 带 4–5 人交付](#resume-16) · [BEHAVIOR-01 岗位价值与能力匹配](#behavior-01)
...
- **危险回答**：“在华为是螺丝钉、自由度低、会的太少”“小鹏现在很不稳定”；过度讨论家庭安排；把组织调整说成唯一原因。
...
<a id="behavior-01"></a>
#### BEHAVIOR-01｜你能为高级训练 Infra 岗位带来什么价值？（P1，10 分钟）
...
- **直接回答（45–60 秒）**：
  ...
  >
  > 这些经历让我能从模型计算、通信、显存一直追到 rollout 和训练数据，判断瓶颈在哪里、该改哪一层、怎样验证。我希望下一岗位继续对核心模块承担明确责任，把性能和正确性优化沉淀为可复用的框架能力。

- **项目证据或知识边界**：当前工作年限约 3 年多，需要用具体项目说明技术深度和影响范围；补齐服务用户数、GPU-hours、默认 recipe/主干贡献等业务影响数据。
- **高概率追问**：你能独立负责哪个模块？如何衡量业务影响？哪些能力可以迁移到新团队？
...
- **面试官意图**：评估技术深度、工程判断和影响范围是否匹配岗位职责。

- **危险回答**：只用学历/大厂背景论证；只罗列框架名称而不讲具体贡献；虚构团队影响。
...
自我介绍 → X1/Fully Async/AReaL 三选一 → Ownership → 职业选择 → 岗位价值与能力匹配。
...
**本轮不建议问**：只问晋升、加班等应由 HR 或主管回答的问题；也不要把宝贵时间用在组内同事难以回答的宏观公司战略上。
...
**本轮不建议问**：不要沉入过细的底层实现，除非主管主动深入；可以询问评价机制，但不要在技术面索要晋升保证。
