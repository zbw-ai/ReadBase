...
> **怎么用**：面试官通常沿简历追问。先定位到教育背景、工作技能或具体项目，再进入唯一题目正文；题尾可返回本 Part 或本控制台，浏览器返回按钮、macOS `⌘ + [`、Windows/Linux `Alt + ←` 可回到刚才的位置。
...
反问不是展示知识，而是获取决策信息：**一面判断实际工作和协作方式，二面判断直属主管与岗位 ownership，三面判断部门方向和 Leader 是否值得长期跟随。** 不要照单全问；结合面试中已经获得的信息选 1–2 题，并沿对方的回答追问一次。
...
2. **技术栈与岗位落点**：Megatron、FSDP/DeepSpeed、verl/AReaL 在团队里分别是直接使用、深度二开还是自研？这个岗位最可能负责哪一层？
...
1. **长期技术方向**：未来两三年，您认为大模型、具身和多模态训练 Infra 最决定上限的能力是什么？团队愿意长期投入建设哪一层？
